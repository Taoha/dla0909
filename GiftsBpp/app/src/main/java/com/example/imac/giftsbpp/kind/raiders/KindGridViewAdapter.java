...
public class KindGridViewAdapter extends BaseAdapter {
    ...
    public KindGridViewAdapter(Context context) {
...
//        hodler.nickname.setText(bean.getData().getChannel_groups().get(i).getName());
        Picasso.with(context).load(bean.getData().getChannel_groups().get(0).getChannels().get(i).getCover_image_url()).into(hodler.avatar);
